...
// processing -> failed. processing 상태가 아니거나 이 명령을 선점한 Agent가 아니면 거부한다.
export default createHandler({
  async POST(request, response) {
    const { agentId } = requireAgent(request);
    const { errorMessage } = parseBody(request);
    const command = await markCommandFinished(request.query.id, 'failed', errorMessage ?? null, agentId);
